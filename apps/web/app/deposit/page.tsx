...
import { useApproveErc20, useDeposit, NATIVE_TOKEN } from '../../lib/debank'
...
export default function DepositPage() {
  ...
  const { hash, error, isPending, receipt, depositEth, depositErc20 } = useDeposit()
  const { approve, isPending: approving, hash: approveHash } = useApproveErc20(token)
  ...
  return (
    <section className="space-y-4">
      ...
      <form onSubmit={onSubmit} className="space-y-3 max-w-md">
        ...
        <button disabled={isPending || approving} className="px-4 py-2 rounded bg-blue-600 hover:bg-blue-500 disabled:opacity-50" type="submit">
          {isPending || approving ? 'Submitting...' : 'Deposit'}
        </button>
      </form>
      <TxStatus loading={isPending || approving} hash={hash || approveHash} error={error?.message} />
    </section>
  )
}"use client"
